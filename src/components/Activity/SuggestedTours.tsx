...
const SuggestedTours: React.FC = () => {
  const [tours, setTours] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isRecommended, setIsRecommended] = useState(false); // Track xem đang hiển thị recommendations hay popular
  ...
  useEffect(() => {
    const fetchSuggestedTours = async () => {
      try {
        // Nếu user đã đăng nhập, thử lấy recommendations
        if (userData) {
          try {
        ...
        // Nếu không có recommendations hoặc user chưa đăng nhập, lấy tour phổ biến
        // Tour phổ biến: sắp xếp theo số lượng booking hoặc featured
        const popularTours = allTours
          .map((tour) => {
            // Tính điểm phổ biến dựa trên:
            // - Featured: +10 điểm
            // - Số lượng reviews: +1 điểm mỗi review
            // - Rating trung bình: +2 điểm mỗi sao
            const reviewCount = tour.reviews?.length || 0;
            const averageRating =
              reviewCount > 0 && tour.reviews
                ? tour.reviews.reduce(
            ...
            const popularityScore =
              (tour.featured ? 10 : 0) + reviewCount * 1 + averageRating * 2;

            return { ...tour, popularityScore, averageRating };
          })
              ...
              <div className="relative">
                ...
                <span
                  className={`absolute top-2 left-2 text-white text-xs font-semibold px-3 py-1 rounded-full flex items-center gap-1 ${
                    isRecommended ? "bg-purple-600" : "bg-orange-500"
                  }`}
                >
                  {isRecommended ? (
                    <>
